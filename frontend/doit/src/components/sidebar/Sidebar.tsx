...
import { useIsMobile } from "@/hooks/sidebar/useMobile"
...
import { sidebarMenuButtonVariants } from "@/utils/components/sidebar/sidebarUtils";

const SIDEBAR_COOKIE_NAME = "sidebar:state"
const SIDEBAR_COOKIE_MAX_AGE = 60 * 60 * 24 * 7
const SIDEBAR_WIDTH = "16rem"
const SIDEBAR_HEIGHT_MOBILE = "7rem"
    ...
    (
        ...
        if (isMobile) {
            return (
                <div
                    ref={ref}
                    className="fixed bottom-0 left-0 right-0 z-50 bg-sidebar text-sidebar-foreground"
                    style={{ height: SIDEBAR_HEIGHT_MOBILE }}
                    {...props}
                >
            ...
            >
                ...
                <div
                    className={cn(
                        ...
                        variant === "floating" || variant === "inset"
                            ? "p-4 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4)_+16px)]"
...
>(({ className, onClick, ...props }, ref) => {
    ...
    return (
        <Button
            ...
            variant="ghost"
            size="icon"
            className={cn("!h-9 !w-9 m-6 rounded-full [&_svg]:!size-5", className)}
            onClick={(event) => {
                onClick?.(event)
                toggleSidebar()
            }}
...
>(({ className, ...props }, ref) => {
    ...
    return (
        <ul
            ...
            className={cn(
                "flex min-w-0 gap-1",
                isMobile ? "flex-row items-center justify-around w-full mt-0" : "w-full mt-4 flex-col",
                className
            )}
            {...props}
        />
    ...
    return (
        <li
            ...
            className={cn(
                ...
                isMobile && "flex flex-col items-center justify-center w-1/3",
                className
            )}
            {...props}
        />
        ...
        const button = (
            <Comp
                ...
                className={cn(
                    ...
                    isMobile ? "px-4 [&>svg]:size-5" : "px-4 [&>svg]:size-4",
                    className
                )}
                {...props}
            />
