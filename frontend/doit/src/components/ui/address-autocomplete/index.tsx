...
import AddressDialog from "@/components/ui/address-autocomplete/AddressDialog";
...
function AddressAutoCompleteInput(props: CommonProps) {
    ...
    >
    <div className="flex w-full items-center justify-between rounded-3xl border bg-background ring-offset-background text-sm focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2">
    <CommandPrimitive.Input
        value={searchInput}
    onValueChange={setSearchInput}
    onBlur={close}
    onFocus={open}
    placeholder={placeholder || "Inserire un indirizzo"}
    className="w-full p-3 rounded-3xl outline-none bg-background"
        />
        </div>
    {searchInput !== "" && !isOpen && !selectedPlaceId && showInlineError && (
        ...
        messages={["Selezionare un indirizzo valido dalla lista"]}
        />
    )}
    ...
    >
        ...
        {searchInput === ""
            ? "Per favore inserire un indirizzo"
            : "No address found"}
        </div>
    )}
        </CommandEmpty>
